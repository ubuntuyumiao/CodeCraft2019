	  ...
	  //先从最小id道路开始
	    ...
	    {  
	       ...
	       sch_car_road= sch_most_prior(car_array, car_dict_,
			   ...
			   <<std::endl;
//                           debug_dir_tocross(road_dict_, road_array,cross_dict_,cross_array,dijk_graph); 
//                            sleep(1);
			   }
 #endif
...
{
    	      for(unsigned int test_cross=55;test_cross<=55;test_cross++)
	      {
		/******需要调度的路口id升序存于cur_cross_road中    起始下标为 array_offset******/
		int cur_cross_road[4];
		...
		if(road_array_[road_sub].end==cross_dict_[test_cross])
		 std::cout <<cross_array_[test_cross].id<<"<-------"<<cur_cross_road[test_offset]<<"-------"<< cross_array_[test_cross].prior_uproad<<"---" 
		 << cross_array_[test_cross].prior_downroad<<"---" << cross_array_[test_cross].prior_leftroad<<"---" << cross_array_[test_cross].prior_rightroad<<"---" 
		 <<road_array_[road_sub].start<<std::endl;
